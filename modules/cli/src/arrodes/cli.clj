...
(defn- decode-text [bytes path]
  (try
    ...
    (catch Throwable _
      (u/fail! :invalid-input "Attached non-image file is not valid UTF-8" {:path path}))))
...
(defn- attachment [cwd value]
  (let [path (u/path (u/resolve-path cwd value))]
    (u/check! (and (Files/exists path (make-array LinkOption 0))
    ...
    (let [size (Files/size path)]
      (u/check! (<= size max-input-bytes) :input-too-large
        ...
        (let [read (.read reader buffer 0 (alength buffer))]
          (if (neg? read)
            ...
            (let [next-total (+ total read)]
              (u/check! (<= next-total max-input-bytes) :input-too-large
...
(defn- prompt-value [options cwd]
  (let [attachments (mapv #(attachment cwd %) (:files options))
        ...
        _ (u/check! (<= attachment-bytes max-input-bytes) :input-too-large
...
(defn- resolve-session-id [api runtime candidate]
  (let [sessions (:sessions (dispatch! api runtime "session.list" {}))
    ...
    (cond
      ...
      (empty? matches) (u/fail! :session-not-found "No session matches the requested ID" {:session-id candidate})
      :else (u/fail! :ambiguous-session "Session ID prefix is ambiguous"
...
(defn- wait-operation! [api runtime receipt]
  (let [operation-id (:id receipt)]
    (u/check! (and (string? operation-id) (not (str/blank? operation-id))) :invalid-operation
...
(defn- assistant-text [api runtime session-id operation]
  (or (not-empty (u/text-content (or (get-in operation [:result :message/content])
      ...
      (->> (:entries (dispatch! api runtime "session.entries" {:session-id session-id :branch? true}))
           ...
           last :data :message/content u/text-content not-empty)
      ""))
...
(defn- run-runtime-mode! [options wire-out diagnostics]
  (let [api (resolve-api)
        open-options (runtime-options options)
      ...
      (finally
        (let [report ((:close! api) runtime)]
          (when-not (= :closed (:status report))
            (u/fail! :shutdown-incomplete "Runtime did not finish shutting down"
                     {:cleanup report})))))))
  ...
  (let [json-mode? (or (some #{"--mode=json"} arguments)
    ...
    (try
      ...
      (catch Throwable error
        (if json-mode?
          (try
            (println (json/write-str {:type "error" :error (u/redact (u/error-map error))}))
