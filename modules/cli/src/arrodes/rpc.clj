...
(defn- option-int [options key default minimum maximum]
  (let [value (get options key default)]
    (u/check! (and (integer? value) (<= minimum value maximum)) :invalid-options
...
(defn json-str
  ...
  [public-value value]
  (json/write-str (-> (public-value value) u/redact wire-value)))
...
(defn- reserve-host! [context id promise]
  (locking (:pending-host context)
    (u/check! (< (count @(:pending-host context)) (:host-limit context))
    ...
    (try
      (loop []
        (when (or @(:closing? context) (.isInterrupted (Thread/currentThread)))
          (u/fail! :cancelled "Host request was cancelled"
                   {:host-request-id id :local-host-cancel? true}))
        (let [remaining (- deadline (System/currentTimeMillis))]
          (when-not (pos? remaining)
            (u/fail! :host-timeout "Host did not answer in time"
          ...
          (let [answer (deref promise (long (min remaining 1000)) ::waiting)]
            (if (= ::waiting answer)
              ...
              (case (:status answer)
                :ok (:result answer)
                :cancelled (u/fail! :cancelled "Host request was cancelled" {:host-request-id id})
                :error (let [remote (:error answer)]
                         (u/fail! (keyword (or (:code remote) "host-error"))
                                  (or (:message remote) "Host request failed")
                                  (assoc (or (:data remote) {}) :host-request-id id)))
                (u/fail! :invalid-host-response "Host returned an invalid response" {:host-request-id id}))))))
      (finally
...
(defn- host-request! [context request]
  (let [kind (let [value (:kind request)] (if (string? value) (keyword value) value))]
    (u/check! (contains? supported-host-kinds kind) :unsupported-host-request
...
(defn- initialize! [context params]
  (u/check! (map? params) :invalid-params "Initialize params must be an object" {})
  (locking (:initialize-lock context)
    (u/check! (= :new @(:initialize-state context)) :already-initialized
...
(defn- invoke-request! [context request]
  (let [method (:method request)
    ...
    (u/check! (= :open @(:initialize-state context)) :not-initialized
              "Send initialize and await its response before invoking commands"
              {:state @(:initialize-state context)})
    (u/check! (map? params) :invalid-params "Request params must be an object" {})
...
(defn- submit-request! [context request]
  (let [id (:id request)
    ...
    (u/check! (and (string? id) (not (str/blank? id))) :invalid-request
              "Request id must be a non-empty string" {})
    (u/check! (and (string? method) (not (str/blank? method))) :invalid-request
    ...
    (let [responded (AtomicBoolean. false)
      ...
      (locking (:inflight context)
        (u/check! (not (contains? @(:inflight context) id)) :duplicate-id
...
(defn- handle-message! [context message]
  (u/check! (map? message) :invalid-request "Each JSONL record must be an object" {})
  (case (:type message)
    ...
    (u/fail! :invalid-request "Unknown transport message type"
             {:type (:type message)
