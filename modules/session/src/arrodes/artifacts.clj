...
(defn- content-bytes [content]
  (cond
    ...
    :else (util/fail! :invalid-artifact-content "Artifact content must be a string or byte array"
...
(defn- infer-kind [content requested]
  (let [kind (or requested (if (bytes? content) :binary :text))
    ...
    (util/check! (contains? artifact-kinds kind) :invalid-artifact-kind
                 "Artifact kind must be :text, :edn, or :binary" {:kind kind})
    (when (not= (= :binary kind) (bytes? content))
      (util/fail! :invalid-artifact-content
...
(defn- private-dir! [path]
  (let [existed? (Files/exists path (make-array LinkOption 0))]
    ...
    (util/check! (not (Files/isSymbolicLink path)) :insecure-directory
...
(defn put!
  ...
  (let [kind (infer-kind content (:kind opts))
        ...
        _ (when artifact-name
            (util/check! (and (string? artifact-name) (not (str/blank? artifact-name))) :invalid-artifact-name
        ...
        (store/store-read store
          (fn [connection]
            ...
            (let [found (find-artifact connection id)]
              (util/check! found :artifact-not-found "Artifact does not exist" {:artifact-id id})
              (util/check! (= sid (:session-id found)) :artifact-forbidden
...
(defn read!
  ...
  (let [descriptor (get-artifact store sid id)]
    (util/check! (:available? descriptor) :artifact-unavailable
                 "Artifact content is unavailable" {:artifact-id id :session-id sid})
    (let [bytes (artifact-bytes store descriptor)]
      (util/check! bytes :artifact-unavailable "Artifact content is unavailable"
...
(defn- serializable! [value field]
  (util/check! (bounded-edn-shape? value) :non-serializable-result
  ...
  (try
    ...
    (catch Throwable error
      (util/fail! :non-serializable-result "Result data is not durable EDN"
                  {:field field :class (some-> value class .getName)
...
(defn- normalize-result [store connection sid descriptor]
  (let [kind (if (string? (:kind descriptor)) (keyword (:kind descriptor)) (:kind descriptor))]
    (util/check! (contains? result-kinds kind) :invalid-result-kind
                 "Result kind must be :inline, :artifact, or :live" {:kind kind})
    (util/check! (string? (or (:content descriptor) "")) :invalid-result
                 "Result content must be a string" {:field :content})
    (util/check! (map? (or (:details descriptor) {})) :invalid-result
                 "Result details must be a map" {:field :details})
    (serializable! (or (:details descriptor) {}) :details)
    (when (= :inline kind)
      (util/check! (contains? descriptor :value) :invalid-result
    ...
    (when (= :artifact kind)
      (let [artifact-id (:artifact-id descriptor)
        ...
        (util/check! found :artifact-not-found "Result artifact does not exist" {:artifact-id artifact-id})
        (util/check! (= sid (:session-id found)) :artifact-forbidden
                     "Result artifact belongs to another session" {:artifact-id artifact-id :session-id sid})
        (util/check! (artifact-content-present? store connection found) :artifact-unavailable
...
(defn- require-result-id! [id]
  (util/check! (and (integer? id) (pos? id) (<= id Long/MAX_VALUE))
               :invalid-result-id
    ...
    (fn [connection]
      ...
      (let [id (require-result-id! id)
        ...
        (if value
          ...
          (util/fail! :result-not-found "Result does not exist"
