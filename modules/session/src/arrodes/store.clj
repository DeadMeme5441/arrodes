...
(defn- encode [value]
  (util/check! (bounded-edn-shape? value) :non-durable-value
  ...
  (let [serialized (binding [*print-length* nil *print-level* nil *print-dup* false]
    ...
    (try
      ...
      (catch Throwable error
        (util/fail! :non-durable-value "Value cannot be persisted as EDN"
...
(defn- require-uuid! [value field]
  (util/check! (uuid? value) :invalid-id "Expected a UUID string" {:field field :value value})
  value)
...
(defn- ensure-open! [store]
  (util/check! (and store (not @(:closed? store))) :store-closed "Store is closed" {}))
...
(defn- private-dir! [path]
  (let [target (util/path path)
    ...
    (util/check! (not (Files/isSymbolicLink target)) :insecure-directory
    ...
    (try
      (if existed?
        (let [mode (PosixFilePermissions/toString (Files/getPosixFilePermissions target (make-array java.nio.file.LinkOption 0)))]
          (util/check! (= "------" (subs mode 3)) :insecure-directory
...
(defn- write-content-addressed! [store sha bytes]
  (let [target (artifact-storage-path store sha)
    ...
    (private-dir! parent)
    (util/check! (not (Files/isSymbolicLink target)) :artifact-corrupt
    ...
    (util/check! (= sha (util/sha256 (Files/readAllBytes target))) :artifact-corrupt
...
(defn- canonical-database-path [path]
  (let [target (.normalize (.toAbsolutePath (util/path path)))
    ...
    (util/check! (and parent filename) :invalid-store-path
...
(defn- acquire-owner! [db-path]
  (let [target (util/path (str db-path ".lock"))]
    ...
    (util/check! (not (Files/isSymbolicLink target)) :insecure-database
    ...
    (let [channel (FileChannel/open
      ...
      (try
        (util/check! (Files/isRegularFile target
        ...
        (let [owner-lock (try
          ...
          (util/check! owner-lock :store-in-use
...
(defn- prepare-database-file! [path]
  (let [target (util/path path)]
    ...
    (doseq [sidecar [(util/path (str path "-wal")) (util/path (str path "-shm"))]]
      (util/check! (not (Files/isSymbolicLink sidecar)) :insecure-database
...
(defn- tighten-store-files! [store]
  (when-let [path (:path store)]
    (doseq [candidate (map util/path [path (str path "-wal") (str path "-shm")])]
      (util/check! (not (Files/isSymbolicLink candidate)) :insecure-database
...
(defn- migrate! [^Connection connection]
  (let [current (long (or (scalar connection "PRAGMA user_version" []) 0))]
    (util/check! (<= current schema-version) :schema-too-new
...
(defn- require-session [^Connection connection sid]
  ...
  (or (find-session connection sid)
      (util/fail! :session-not-found "Session does not exist" {:session-id sid})))
...
(defn create-session! [store opts]
  (let [snapshot (session-model/new-snapshot opts)]
    (require-uuid! (:id snapshot) :session-id)
    (util/check! (contains? statuses (:status snapshot)) :invalid-session "Invalid session status" {:status (:status snapshot)})
    (transact! store
      (fn [connection]
        (util/check! (nil? (find-session connection (:id snapshot))) :session-exists
...
(defn- validate-entry! [^Connection connection sid {:keys [id parent-id kind data created-at]}]
  ...
  (util/check! (contains? entry-kinds kind) :invalid-entry "Invalid entry kind" {:kind kind})
  (util/check! (map? data) :invalid-entry "Entry data must be a map" {:kind kind})
  (util/check! (and (integer? created-at) (not (neg? created-at))) :invalid-entry
               "Entry creation time must be a non-negative integer" {:created-at created-at})
  (when parent-id
    (require-uuid! parent-id :parent-id)
    (util/check! (entry-exists? connection sid parent-id) :invalid-branch
                 "Entry parent is not part of this session" {:session-id sid :parent-id parent-id}))
  (case kind
    :config
    (util/check! (= data (session-model/normalize-config data)) :invalid-entry
    ...
    (let [kept (:first-kept-entry-id data)
      ...
      (util/check! (contains? ancestors kept) :invalid-entry
                   "Compaction retained entry must be on its preceding active path"
                   {:first-kept-entry-id kept})
      (util/check! (string? (:summary data)) :invalid-entry
                   "Compaction summary must be a string" {})
      (util/check! (map? (or (:usage data) {})) :invalid-entry
    ...
    (do
      ...
      (when-let [from-id (:from-id data)]
        ...
        (util/check! (entry-exists? connection sid from-id) :invalid-entry
    ...
    (let [entry-id (:entry-id data)]
      ...
      (util/check! (entry-exists? connection sid entry-id) :invalid-entry
...
(defn- normalize-operation [sid prior operation]
  (let [allowed #{:id :session-id :kind :status :created-at :finished-at :result :error}
    ...
    (util/check! (nil? unknown) :invalid-operation
                 "Operation contains unsupported fields" {:fields (vec unknown)})
    (when prior
      (util/check! (= sid (:session-id prior)) :operation-forbidden
      ...
      (doseq [field [:session-id :kind :created-at]]
        (when (contains? operation field)
          (util/check! (= (get prior field) (get operation field)) :invalid-operation
    ...
    (let [operation (if prior
      ...
      (util/check! (= sid (:session-id operation)) :invalid-operation
                   "Operation belongs to another session" {:operation-id (:id operation)})
      (util/check! (contains? operation-kinds (:kind operation)) :invalid-operation
                   "Invalid operation kind" {:kind (:kind operation)})
      (util/check! (contains? operation-statuses (:status operation)) :invalid-operation
                   "Invalid operation status" {:status (:status operation)})
      (util/check! (and (integer? (:created-at operation)) (not (neg? (:created-at operation))))
                   :invalid-operation "Operation creation time must be a non-negative integer" {})
      (when (contains? operation :finished-at)
        (util/check! (and (integer? (:finished-at operation))
                          (not (neg? (:finished-at operation))))
                     :invalid-operation "Operation finish time must be a non-negative integer" {}))
      (when prior
        (util/check! (contains? (get operation-transitions (:status prior) #{})
                                (:status operation))
...
(defn- insert-event! [^Connection connection sid event]
  (let [id (or (:id event) (util/id))
    ...
    (util/check! (keyword? (:type event)) :invalid-event
                 "Event type must be a keyword" {:type (:type event)})
    (util/check! (map? data) :invalid-event "Event data must be a map" {})
    (util/check! (and (integer? time) (not (neg? time))) :invalid-event
    ...
    (when-let [oid (:operation-id event)]
      ...
      (let [operation (find-operation connection oid)]
        (util/check! operation :operation-not-found
                     "Event operation does not exist" {:operation-id oid})
        (util/check! (= sid (:session-id operation)) :operation-forbidden
...
(defn- normalize-session-changes [snapshot changes]
  (let [allowed #{:name :cwd :head :config :status :metadata :labels}
    ...
    (util/check! (nil? unknown) :invalid-session-change "Unsupported session field" {:fields (vec unknown)})
    (let [result (merge snapshot changes)
      ...
      (util/check! (and (string? (:name result)) (not (str/blank? (:name result)))) :invalid-session "Session name must not be blank" {})
      (util/check! (contains? statuses (:status result)) :invalid-session "Invalid session status" {:status (:status result)})
      (util/check! (map? (:metadata result)) :invalid-session "Session metadata must be a map" {})
      result)))
  ...
  (transact! store
    (fn [connection]
      (let [current (require-session connection sid)
        ...
        (when (some? expected)
          (util/check! (integer? expected) :invalid-revision
                       "Expected revision must be an integer" {:expected expected})
          (util/check! (= (long expected) (:revision current)) :stale-revision
        ...
        (let [start-seq (long (or (scalar connection "SELECT MAX(seq) FROM entries WHERE session_id = ?" [sid]) 0))
              ...
              (loop [remaining raw-entries, parent (:head current), seq (inc start-seq), result []]
                (if-let [raw (first remaining)]
                  (let [entry (merge {:id (util/id)
                    ...
                    (util/check! (nil? (scalar connection "SELECT 1 FROM entries WHERE id = ?" [(:id entry)]))
              ...
              _ (when-let [head (:head changes)]
                  ...
                  (util/check! (entry-exists? connection sid head) :invalid-branch
              ...
              _ (doseq [id (or (:queue-deliver command) [])]
                  ...
                  (util/check! (= 1 (execute-sql! connection "DELETE FROM queue WHERE id = ? AND session_id = ?" [id sid]))
              ...
              _ (doseq [raw (or (:queue-enqueue command) [])]
                  (let [item (merge {:id (util/id) :options {} :created-at now} raw)]
                    ...
                    (util/check! (contains? #{:steering :follow-up} (:kind item)) :invalid-queue-item
                                 "Invalid queue item kind" {:kind (:kind item)})
                    (util/check! (or (string? (:content item)) (vector? (:content item))) :invalid-queue-item
                                 "Queue content must be text or canonical parts" {})
                    (util/check! (map? (or (:options item) {})) :invalid-queue-item
                                 "Queue options must be a map" {})
                    (util/check! (and (integer? (:created-at item))
              ...
              events (mapv #(insert-event! connection sid %) (or (:events command) []))]
...
(defn configure!
  ...
  (let [snapshot (session store sid)
        ...
        config (session-model/normalize-config (util/deep-merge (:config snapshot) config-changes))
...
(defn- copy-value-records! [store ^Connection connection old-sid new-sid entries now]
  (let [wanted (referenced-result-ids entries)
        ...
        _ (util/check! (= wanted found) :result-not-found
        ...
        (mapv (fn [artifact-id]
                (let [row (first (query-sql connection "SELECT * FROM artifacts WHERE session_id=? AND id=?"
                  ...
                  (util/check! row :artifact-not-found
...
(defn- copy-path! [store ^Connection connection source-row source-path opts]
  (let [new-sid (or (:id opts) (util/id))
    ...
    (util/check! (nil? (find-session connection new-sid)) :session-exists
    ...
    (fn [connection]
      (let [source (require-session connection sid)
            ...
            _ (when (some? expected)
                (util/check! (and (integer? expected)
            ...
            _ (util/check! requested :invalid-fork "Cannot fork an empty session" {:session-id sid})
            target (case (or (:position opts) :at)
                     ...
                     :before (let [entry (first (query-sql connection "SELECT * FROM entries WHERE session_id=? AND id=?" [sid requested] entry-row))]
                               (util/check! entry :entry-not-found "Fork entry does not exist" {:entry-id requested})
                               (:parent-id entry))
                     (util/fail! :invalid-fork "Fork position must be :before or :at" {:position (:position opts)}))
  ...
  (store-read store
    (fn [connection]
      ...
      (or (find-operation connection oid)
          (util/fail! :operation-not-found "Operation does not exist" {:operation-id oid})))))
...
(defn events-since [store {:keys [after session-id limit] :or {after 0 limit 1000}}]
  (util/check! (and (integer? after) (not (neg? after)) (<= after Long/MAX_VALUE))
               :invalid-event-cursor "Event cursor must be a non-negative 64-bit integer"
               {:after after})
  (util/check! (and (integer? limit) (pos? limit))
...
(defn- export-artifacts [store connection sid]
  (let [records (query-sql connection
    ...
    (util/check! (<= total max-transfer-artifact-bytes) :session-export-too-large
  ...
  (let [artifact-availability
    ...
    (mapv (fn [descriptor]
            (case (:kind descriptor)
              ...
              :artifact
              (do
                (util/check! (contains? artifact-availability (:artifact-id descriptor))
                             :artifact-not-found
...
(defn- validate-import-artifacts! [records source-sid]
  ...
  (let [ids (mapv #(get-in % [:descriptor :id]) records)
    ...
    (util/check! (<= total max-transfer-artifact-bytes) :session-import-too-large
...
(defn- validate-import-results! [descriptors source-sid artifact-availability]
  (util/check! (vector? descriptors) :invalid-import "Export results must be a vector" {})
  (let [ids (mapv :id descriptors)]
    (util/check! (= (count ids) (count (set ids))) :invalid-import
                 "Export contains duplicate result IDs" {})
    (doseq [descriptor descriptors]
      (util/check! (map? descriptor) :invalid-import "Result descriptor must be a map" {})
      (util/check! (and (integer? (:id descriptor)) (pos? (:id descriptor))) :invalid-import
                   "Result ID must be positive" {:result-id (:id descriptor)})
      (util/check! (= source-sid (:session-id descriptor)) :invalid-import
                   "Result belongs to another exported session" {:result-id (:id descriptor)})
      (util/check! (contains? #{:inline :artifact :live} (:kind descriptor)) :invalid-import
                   "Result has an invalid kind" {:result-id (:id descriptor)})
      (util/check! (boolean? (:available? descriptor)) :invalid-import
                   "Result availability must be boolean" {:result-id (:id descriptor)})
      (util/check! (string? (or (:content descriptor) "")) :invalid-import
                   "Result content must be text" {:result-id (:id descriptor)})
      (util/check! (map? (or (:details descriptor) {})) :invalid-import
                   "Result details must be a map" {:result-id (:id descriptor)})
      (case (:kind descriptor)
        :inline
        (util/check! (contains? descriptor :value) :invalid-import
                     "Inline result is missing its durable value" {:result-id (:id descriptor)})

        :artifact
        (do
          (util/check! (contains? artifact-availability (:artifact-id descriptor)) :invalid-import
                       "Result references a missing artifact"
                       {:result-id (:id descriptor) :artifact-id (:artifact-id descriptor)})
          (util/check! (or (not (:available? descriptor))
                           (get artifact-availability (:artifact-id descriptor)))
                       :invalid-import "Available result references an unavailable artifact"
                       {:result-id (:id descriptor) :artifact-id (:artifact-id descriptor)}))

        :live
        (util/check! (false? (:available? descriptor)) :invalid-import
...
(defn- validate-import! [packet]
  (util/check! (map? packet) :invalid-import "Session export must be a map" {})
  (util/check! (= "arrodes-session" (:format packet)) :invalid-import
               "Unsupported session export format" {:format (:format packet)})
  (util/check! (= 1 (:version packet)) :invalid-import
               "Unsupported session export version" {:version (:version packet)})
  (util/check! (vector? (:entries packet)) :invalid-import
               "Export entries must be a vector" {})
  (util/check! (contains? packet :base-config) :invalid-import
  ...
  (let [snapshot (:session packet)
    ...
    (util/check! (map? (:metadata snapshot)) :invalid-import
                 "Export session metadata must be a map" {})
    (util/check! (vector? (:labels snapshot)) :invalid-import
                 "Export session labels must be a vector" {})
    (require-uuid! (:id snapshot) :session-id)
    (util/check! (= (count ids) (count id-set)) :invalid-import
                 "Export contains duplicate entry IDs" {})
    (util/check! (= (count entries) (count (set (map :seq entries)))) :invalid-import
                 "Export contains duplicate entry sequence numbers" {})
    (doseq [entry entries]
      (util/check! (map? entry) :invalid-import "Export entry must be a map" {})
      (require-uuid! (:id entry) :entry-id)
      (util/check! (= (:id snapshot) (:session-id entry)) :invalid-import
                   "Entry belongs to another session" {:entry-id (:id entry)})
      (util/check! (contains? entry-kinds (:kind entry)) :invalid-import
                   "Export contains an invalid entry kind"
                   {:entry-id (:id entry) :kind (:kind entry)})
      (util/check! (or (nil? (:parent-id entry)) (contains? id-set (:parent-id entry)))
                   :invalid-import "Entry parent is missing"
                   {:entry-id (:id entry) :parent-id (:parent-id entry)})
      (util/check! (and (integer? (:seq entry)) (pos? (:seq entry))) :invalid-import
                   "Entry sequence must be a positive integer"
                   {:entry-id (:id entry) :seq (:seq entry)})
      (util/check! (and (integer? (:created-at entry)) (not (neg? (:created-at entry))))
      ...
      (when-let [parent (:parent-id entry)]
        (let [parent-seq (:seq (get by-id parent))]
          (util/check! (and (integer? parent-seq) (< parent-seq (:seq entry))) :invalid-import
                       "Entry parent must precede its child"
                       {:entry-id (:id entry) :parent-id parent})))
      (util/check! (map? (:data entry)) :invalid-import
                   "Entry data must be a map" {:entry-id (:id entry)})
      (case (:kind entry)
        :config
        (util/check! (= (:data entry)
        ...
        (let [kept (get-in entry [:data :first-kept-entry-id])
          ...
          (util/check! (contains? ancestors kept) :invalid-import
                       "Compaction retained entry must be on its preceding active path"
                       {:entry-id (:id entry) :first-kept-entry-id kept})
          (util/check! (string? (get-in entry [:data :summary])) :invalid-import
                       "Compaction summary must be text" {:entry-id (:id entry)})
          (util/check! (map? (or (get-in entry [:data :usage]) {})) :invalid-import
        ...
        (do
          ...
          (when-let [from-id (get-in entry [:data :from-id])]
            (util/check! (contains? id-set from-id) :invalid-import
                         "Branch summary references a missing entry"
                         {:entry-id (:id entry) :from-id from-id})))

        :label
        (util/check! (contains? id-set (get-in entry [:data :entry-id])) :invalid-import
                     "Label references a missing entry" {:entry-id (:id entry)})

        nil))
    (when-let [head (:head snapshot)]
      (util/check! (contains? id-set head) :invalid-import
    ...
    (let [artifacts (validate-import-artifacts! (:artifacts packet) (:id snapshot))
      ...
      (util/check! (set/subset? referenced present) :invalid-import
                   "History references results missing from the export"
                   {:result-ids (vec (sort (set/difference referenced present)))})
      (util/check! (set/subset? artifact-referenced artifact-present) :invalid-import
