...
(defn prompt-content [prompt]
  (cond
    ...
    :else (util/fail! :invalid-prompt "Prompt must be a string or canonical content parts"
                      {:value-type (some-> prompt class str)})))
...
(defn effective-config [session overrides]
  (let [base (util/deep-merge default-config (:config session))
        config (util/deep-merge base (or overrides {}))
        thinking (:thinking config)]
    (util/check! (keyword? (:provider config)) :invalid-config
                 "Provider must be a keyword" {:provider (:provider config)})
    (util/check! (and (string? (:model config)) (not (str/blank? (:model config))))
                 :invalid-config "Model must be a non-empty string" {:model (:model config)})
    (util/check! (contains? #{:none :minimal :low :medium :high :xhigh :max} thinking)
                 :invalid-config "Unsupported thinking level" {:thinking thinking})
    (util/check! (or (= :all (:tools config))
...
(defn validate-assistant! [assistant]
  (let [calls (vec (:message/tool-calls assistant))
    ...
    (util/check! (or (seq (:message/content assistant)) (seq calls))
                 :empty-provider-response
                 "Provider returned neither content nor tool calls" {})
    (doseq [call calls]
      (util/check! (and (string? (:tool-call/id call))
    ...
    (util/check! (= (count ids) (count (set ids))) :invalid-tool-call
...
(defn- provider-tool-call [call]
  (let [arguments (:tool-call/arguments call)
        arguments (cond
                    ...
                    (map? arguments) (json/write-str arguments)
                    (nil? arguments) "{}"
                    :else (util/fail! :invalid-tool-call
