...
(defn- check-cancelled! [current-context]
  (when (cancelled? current-context)
    (util/fail! :cancelled "Capability invocation was cancelled" {})))
...
(defn- require-regular-file! [^Path path display]
  (util/check! (Files/exists path (make-array LinkOption 0)) :not-found
               (str "Path does not exist: " display) {:path (str path)})
  (util/check! (Files/isRegularFile path (make-array LinkOption 0)) :not-a-file
...
(defn- decode-utf8 [^bytes bytes display]
  (try
    ...
    (catch CharacterCodingException _
      (util/fail! :unsupported-file "File is neither supported image data nor valid UTF-8 text"
                  {:path display}))))
...
(defn- validate-limit [value default maximum label]
  (let [value (or value default)]
    (util/check! (and (integer? value) (pos? value) (<= value maximum)) :invalid-arguments
...
(defn- read-text-page [^Path target display current-context offset limit]
  (let [offset (or offset 1)
        ...
        _ (util/check! (and (integer? offset) (pos? offset)) :invalid-offset
                       "Read offset must be a positive 1-based line number"
                       {:path (str target) :offset offset})
        size (Files/size target)
        _ (util/check! (<= size max-read-file-bytes) :file-too-large
    ...
    (if (zero? size)
      (do
        (util/check! (= 1 offset) :invalid-offset
          ...
          (loop [line-number 1 selected []]
            ...
            (let [line (.readLine reader)]
              (cond
                (nil? line)
                (do
                  (util/check! (or (seq selected) (= 1 offset)) :invalid-offset
  ...
  (let [target (resolve-path cwd path)
    ...
    (if mime
      (let [size (Files/size target)
            _ (util/check! (<= size max-image-bytes) :file-too-large
...
(defn- write-tool [cwd current-context {:keys [path content]}]
  (check-cancelled! current-context)
  (let [target (resolve-path cwd path)]
    (util/check! (string? content) :invalid-arguments "write content must be a string" {:path path})
    (let [bytes (alength (.getBytes content StandardCharsets/UTF_8))]
      (util/check! (<= bytes max-edit-bytes) :file-too-large
...
(defn- validate-edits! [edits path]
  (util/check! (and (vector? edits) (seq edits)) :invalid-arguments
               "edit requires a non-empty edits array" {:path path})
  (util/check! (<= (count edits) max-exact-edits) :invalid-arguments
  ...
  (doseq [[index edit] (map-indexed vector edits)]
    (util/check! (map? edit) :invalid-arguments "Each edit must be an object" {:index index})
    (util/check! (and (string? (:oldText edit)) (not (empty? (:oldText edit)))) :invalid-arguments
                 "Each edit oldText must be a non-empty string" {:index index})
    (util/check! (string? (:newText edit)) :invalid-arguments
  ...
  (let [located
        (mapv (fn [[index {:keys [oldText newText]}]]
                (let [positions (occurrences content (normalize-newlines oldText))]
                  (util/check! (= 1 (count positions)) :edit-mismatch
                               (if (empty? positions)
    ...
    (doseq [[left right] (partition 2 1 ordered)]
      (util/check! (<= (:end left) (:start right)) :overlapping-edits
...
(defn- edit-tool [cwd current-context {:keys [path edits]}]
  ...
  (let [target (resolve-path cwd path)
        _ (require-regular-file! target path)
        size (Files/size target)
        _ (util/check! (<= size max-edit-bytes) :file-too-large
    ...
    (util/check! (<= final-bytes max-edit-bytes) :file-too-large
...
(defn- timeout-millis [timeout]
  (when (some? timeout)
    (util/check! (and (number? timeout) (pos? timeout) (Double/isFinite (double timeout)))
                 :invalid-arguments "timeout must be a finite positive number of seconds"
                 {:timeout timeout})
    (let [millis (* (double timeout) 1000.0)]
      (util/check! (<= millis Integer/MAX_VALUE) :invalid-arguments
...
(defn- run-process [cwd current-context argv command timeout]
  ...
  (let [dir (resolve-path cwd ".")
        _ (util/check! (Files/isDirectory dir (make-array LinkOption 0)) :invalid-cwd
        ...
        process (try (.start (doto (ProcessBuilder. ^java.util.List argv) (.directory (.toFile dir))))
                     (catch Throwable error
                       (util/fail! :process-start-failed
      ...
      (loop []
        ...
        (cond
          ...
          (do (stop-process-tree! process (vals @known-descendants))
              (util/fail! :cancelled "Command execution was cancelled" {:command command}))

          (and timeout-ms (>= (/ (- (System/nanoTime) started) 1000000) timeout-ms))
          (do (stop-process-tree! process (vals @known-descendants))
              (util/fail! :timeout (str "Command timed out after " timeout " seconds")
...
(defn- bash-tool [cwd current-context {:keys [command timeout]}]
  (util/check! (string? command) :invalid-arguments "bash command must be a string" {})
  (let [bash (or (when (Files/isExecutable (Paths/get "/bin/bash" (make-array String 0))) "/bin/bash")
                 (executable-path ["bash"]))]
    (util/check! bash :unsupported-system-function "bash is not available on this system" {})
...
(defn- powershell-tool [cwd current-context {:keys [command timeout]}]
  (util/check! (string? command) :invalid-arguments "powershell command must be a string" {})
  (let [program (executable-path (if (str/starts-with? (str/lower-case (System/getProperty "os.name")) "windows")
    ...
    (util/check! program :unsupported-system-function
    ...
    (Files/walkFileTree
      ...
      (proxy [SimpleFileVisitor] []
        ...
        (visitFile [file attrs]
          ...
          (util/check! (<= (count @files) max-traversed-files) :traversal-limit
...
(defn- glob-matchers [pattern]
  (try
    ...
    (catch Throwable error
      (util/fail! :invalid-pattern (str "Invalid glob pattern: " (ex-message error)) {:pattern pattern}))))
...
(defn- search-files [cwd current-context path]
  (let [root (resolve-path cwd (or path "."))]
    (util/check! (Files/exists root (make-array LinkOption 0)) :not-found
                 (str "Path does not exist: " (or path ".")) {:path (str root)})
    root))

(defn- find-tool [cwd current-context {:keys [pattern path limit]}]
  (util/check! (and (string? pattern) (not (empty? pattern))) :invalid-arguments
...
(defn- grep-tool [cwd current-context {:keys [pattern path glob ignoreCase literal context limit]}]
  ...
  (let [root (search-files cwd current-context path)
        ...
        _ (util/check! (and (integer? context) (<= 0 context 20)) :invalid-arguments
...
(defn- ls-tool [cwd current-context {:keys [path limit]}]
  ...
  (let [dir (resolve-path cwd (or path "."))
        _ (util/check! (Files/exists dir (make-array LinkOption 0)) :not-found
                       (str "Path does not exist: " (or path ".")) {:path (str dir)})
        _ (util/check! (Files/isDirectory dir (make-array LinkOption 0)) :not-a-directory
                          ...
                          entries (conj entries
                                        (str (.getFileName entry)
                                             (when (Files/isDirectory entry (make-array LinkOption 0))
                                               "/")))]
                      (util/check! (<= (count entries) max-directory-entries)
                                   :traversal-limit
