(ns arrodes.resources
  "Trusted settings, project context, discoverable resources, and attributable Clojure extensions."
...
(defn- fail! [code message data] (u/fail! code message data))
      ...
      (let [next-path (conj path key)
        ...
        (when (and (re-find secret-setting-key key-name)
          ...
          (fail! :settings/secret-forbidden
                 "Provider credentials belong in the private auth store, not settings"
                 {:path next-path}))
...
(defn activate!
  "Activates every discovered extension into a registry. Registration is rolled back on any failure."
...
(defn close!
  "Tears down all attributable extension effects and closes this manager. Idempotent."
  [manager]
  (locking (:lock manager)
