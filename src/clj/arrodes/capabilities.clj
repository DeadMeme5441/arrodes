...
(defrecord Registry
  [session-id cwd store config emit! get-session namespace capabilities capability-stacks order
   ...
   evaluation-lock repl-history live-values resources closed?])

(declare register! register-restorable! restore! invoke! invoke-value! catalog selected-catalog
         result-value artifact-value close!)

(defn- ensure-open! [registry]
  (value/check! (not @(:closed? registry)) :registry-closed
...
(defn- validation-error! [path message data]
  (value/fail! :invalid-arguments message (merge {:path path} data)))
  ...
  (cond
    ...
    (try
      (let [parsed (json/read-str arguments :key-fn keyword)]
        (value/check! (map? parsed) :invalid-arguments "Tool arguments JSON must decode to an object" {})
        parsed)
      (catch clojure.lang.ExceptionInfo error (throw error))
      (catch Throwable error
        (value/fail! :invalid-arguments (str "Tool arguments are not valid JSON: " (ex-message error)) {})))
    (nil? arguments) {}
    :else (value/fail! :invalid-arguments "Tool arguments must be a map or JSON object string"
...
(defn- register-entry! [registry descriptor]
  (locking registry
    ...
    (let [{:keys [name description parameters execution permission owner replace? replace-owner?]} descriptor
      ...
      (value/check! (and (string? name) (not (str/blank? name))) :invalid-capability
                   "Capability name must be a non-empty string" {})
      (value/check! (string? (or description "")) :invalid-capability
                   "Capability description must be a string" {:name name})
      (value/check! (map? parameters) :invalid-capability
                   "Capability parameters must be a JSON Schema map" {:name name})
      (value/check! (fn? (:fn descriptor)) :invalid-capability
                   "Capability descriptor requires a function" {:name name})
      (value/check! (contains? #{:parallel :sequential :exclusive} execution) :invalid-capability
                   "Capability execution must be parallel, sequential, or exclusive"
                   {:name name :execution execution})
      (value/check! (or (contains? #{:read :write :execute} permission) (fn? permission))
                   :invalid-capability "Capability permission must be read, write, execute, or a function"
                   {:name name})
      (when current
        (value/check! replace? :duplicate-capability
                     (str "Capability already registered: " name) {:name name :owner (:owner current)})
        (value/check! (or (= owner (:owner current)) replace-owner?) :capability-owner-mismatch
...
(defn add-hook!
  ...
  (locking registry
    (ensure-open! registry)
    (value/check! (contains? hook-points point) :invalid-hook-point
    ...
    (let [hook (merge {:order 0 :owner "anonymous"} descriptor {:point point})
      ...
      (value/check! (and (string? id) (not (str/blank? id))) :invalid-hook "Hook id must be a non-empty string" {})
      (value/check! (string? owner) :invalid-hook "Hook owner must be a string" {:id id})
      (value/check! (integer? order) :invalid-hook "Hook order must be an integer" {:id id})
      (value/check! (fn? (:fn hook)) :invalid-hook "Hook descriptor requires a function" {:id id})
      (value/check! (not-any? #(= id (:id %)) (mapcat val @(:hooks registry))) :duplicate-hook
...
(defn catalog
  "Return public function descriptors and their callable symbols in stable order."
  [registry]
  (ensure-open! registry)
  (let [all @(:capabilities registry)]
    (->> @(:order registry)
         (keep (fn [name]
                 (when-let [descriptor (get all name)]
                   (assoc (public-descriptor descriptor)
                          :symbol (str (get @(:wrapper-vars registry) name))))))
         (sort-by :name)
         vec)))

(defn set-tools!
  "Set the functions available through this session's REPL to :all or selected names."
  [registry selection]
  (ensure-open! registry)
  (value/check! (or (= :all selection)
  ...
  (when (vector? selection)
    (value/check! (= (count selection) (count (distinct selection)))
                 :invalid-tool-selection "Tool selection contains duplicate names"
                 {:tools selection})
    (doseq [name selection]
      (value/check! (contains? @(:capabilities registry) name) :unknown-capability
                   (str "Unknown capability in selection: " name) {:name name})))
  (reset! (:tool-selection registry) selection)
  registry)

(defn selected-catalog
  "Return the selected REPL function catalog. This is not a provider tool schema."
  ([registry] (selected-catalog registry
                               (or (:tool-selection *invocation-context*)
...
(defn- mutation-lock [registry descriptor arguments]
  (when-let [key-source (:mutation-key descriptor)]
    (let [key (if (fn? key-source) (key-source arguments) key-source)]
      (value/check! (some? key) :invalid-mutation-key
...
(defn- with-execution-lock [registry descriptor arguments context thunk]
  (let [stack (or (.get invocation-stack) [])
    ...
    (when (and (seq enclosing) (= :exclusive requested) (not enclosed-by-exclusive?))
      (value/fail! :nested-exclusive-invocation
  ...
  (when-let [validator (:validate descriptor)]
    (let [outcome (try (validator arguments context)
      ...
      (cond
        ...
        (string? outcome) (value/fail! :invalid-arguments outcome {:name (:name descriptor)})
        (map? outcome) (value/fail! :invalid-arguments
                                   (or (:message outcome) "Capability arguments failed validation") outcome)
        :else (value/check! outcome :invalid-arguments "Capability arguments failed validation"
                           {:name (:name descriptor)})))))
...
(defn- check-permission! [descriptor context arguments]
  (when (fn? (:permission descriptor))
    (let [allowed? (try ((:permission descriptor) context arguments)
      ...
      (value/check! allowed? :permission-denied
...
(defn- explicit-content [name value fallback]
  (let [value (if (nil? value) fallback value)]
    (value/check! (or (string? value) (canonical-content-parts? value))
...
(defn- normalized-result [name returned]
  (if (and (map? returned)
    ...
    (let [details (or (:details returned) {})]
      (value/check! (map? details) :invalid-capability-result
...
(defn- failure-result [name error phase]
  {:value nil
   ...
   :details (assoc (value/error-map error) :capability name :phase phase)
   :error? true})

(defn- invocation-context [registry call-id options]
  ;; Observers run in the caller's bindings, never in the evaluated program's
  ;; *out*/*err*. Otherwise rendering an output event can capture itself.
  (merge (:context options)
         ...
         (into {} (keep (fn [key]
                          (when-let [callback (get options key)]
                            [key (bound-fn [event] (callback event))])))
               [:on-progress :on-event])))

(defn- publish! [registry context type data]
  (let [event {:session-id (:session-id registry) :operation-id (:operation-id context)
    ...
    (when-let [callback (:on-event context)]
      (try (callback committed) (catch Throwable _ nil)))
    committed))
...
(defn evaluate!
  ...
  (let [context (invocation-context registry (or (:id options) (util/id)) options)
    ...
    (let [raw (try
                (with-acquired
                  (:evaluation-lock registry) context
                  #(binding [*invocation-context* context]
...
(defn invoke!
  ...
  as attributed normalized results with retained native values."
  ...
  (let [call-id (:id call)
    ...
    (publish! registry context :capability/started
              {:name initial-name
               :arguments (if (durable-value? (:arguments call))
                            (:arguments call) (bounded-native-print (:arguments call)))})
    (let [raw
          (try
            (cancellation-check! context)
            (let [arguments (parse-arguments (:arguments call))
                  ...
                  _ (value/check! (map? transformed) :invalid-hook-result
              ...
              (value/check! descriptor :unknown-capability (str "Unknown capability: " name) {:name name})
              (let [selection (or (:tool-selection context) @(:tool-selection registry))]
                (value/check! (or (= :all selection) (some #{name} selection))
                             :capability-disabled "Function is not enabled in this session"
                             {:name name}))
              (value/check! (map? final-arguments) :invalid-arguments
              ...
              (let [returned (binding [*invocation-context* invocation-context]
                ...
                (let [hooked (apply-hooks registry :after-invoke invocation-context normalized)]
                  (value/check! (map? hooked) :invalid-hook-result
   ...
   (let [context *invocation-context*]
     (invoke-value! registry name arguments
                    (cond-> {}
                      ...
                      (:on-event context) (assoc :on-event (:on-event context))
                      context (assoc :context
                                     (-> context
                                         (dissoc :registry :descriptor :name
                                                 :call-id :cancelled? :on-progress :on-event)
...
(defn- read-artifact-completely [registry id]
  (loop [offset 1
    ...
    (let [page (artifacts/read! (:store registry) (:session-id registry) id
          ...
          _ (value/check! (<= (:bytes descriptor) max-artifact-helper-bytes)
                         :artifact-too-large
...
(defn create!
  ...
  (value/check! store :invalid-store "Capability registry requires a store" {})
  (let [namespace (symbol (str "arrodes.session."
                               (str/replace session-id #"[^A-Za-z0-9_]" "_")))
        ns-object (locking namespace-lock
                    (value/check! (nil? (find-ns namespace)) :session-namespace-in-use
    ...
    (let [errors (volatile! [])
          released (try
                     (artifacts/release-live-results! (:store registry)
                                                      (:session-id registry))
                     (catch Throwable error
                       (vswap! errors conj (value/error-map error))
                       0))]
