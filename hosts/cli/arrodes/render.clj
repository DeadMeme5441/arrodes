...
(defn- event-type-name [event]
  (let [type (or (:type event) (:event/type event) :event)]
    (str/lower-case
     (if (or (keyword? type) (symbol? type))
       (if-let [scope (namespace type)]
         (str scope "/" (name type))
         (name type))
       (str type)))))
  ...
  ([theme event colors?]
   (let [event (unwrap-event event)
     ...
     (cond
       ...
       (str/starts-with? type "evaluation/")
       [(paint theme :tool
               (if (str/ends-with? type "/started")
                 (str "eval> " (safe-text (or (:source data) "")))
                 (str "eval< "
                      (if (:error? data) "failed" "completed")
                      (when-let [text (not-empty (safe-text (or (:content data) "")))]
                        (str ": " text))))
               colors?)]
       (str/starts-with? type "capability/")
       [(paint theme :tool
               (str "capability"
                    (if (str/ends-with? type "/started") "> " "< ")
                    (or tool-name "capability")
                    (if (str/ends-with? type "/started")
                      (when (seq (:arguments data)) (str " " (pr-str (:arguments data))))
                      (str " " (if (:error? data) "failed" "completed")
                           (when-let [text (not-empty (safe-text (or (:content data) "")))]
                             (str ": " text)))))
               colors?)]
