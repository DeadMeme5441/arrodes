...
(defn- normalize-content-part [part]
  (if (and (map? part)
           (contains? #{"text" "image"} (:part/type part)))
    (update part :part/type keyword)
    part))
(defn- content-value [value]
  (let [value (if (vector? value) (mapv normalize-content-part value) value)]
    (value/check! (or (and (string? value) (not (str/blank? value)))
                      (and (vector? value) (seq value) (every? map? value)))
                  :invalid-params
                  "Content must contain text or content parts" {})
    value))
(defn- content [params]
  (content-value (:prompt params)))
(defn- queue-content [params]
  (content-value (:content params)))
...
(defn- result-view [descriptor]
  (if (= :inline (:kind descriptor))
    (let [{:keys [writer buffer truncated?]} (u/bounded-writer (* 32 1024))]
      ;; JSON cannot distinguish :status from "status", or preserve ratios and
      ;; symbols. Supply a bounded native representation rather than guessing
      ;; those types back in the host. Apply the same public redaction policy.
      (binding [*out* writer *print-length* nil *print-level* nil]
        (pr (value/redact (:value descriptor))))
      (assoc descriptor :value-edn (str buffer) :value-truncated? @truncated?))
    descriptor))
...
(defn- import-file [file]
  (with-open [in (io/input-stream file)]
    (let [limit (* 32 1024 1024)
      ...
      (value/check! (<= (alength bytes) limit) :invalid-import "Session import exceeds 32 MiB" {})
  ...
  (try
            ...
            (assoc metadata :entries
                   (mapv (fn [line]
                           (let [entry (json/read-str line :key-fn keyword)]
                             (value/check! (= "entry" (:type entry)) :invalid-import "Invalid session record" {})
    ...
    (catch Exception error
      (if (:error/code (ex-data error))
        (throw error)
        (value/fail! :invalid-import "Session data could not be decoded" {:reason (ex-message error)})))))
       ...
       (apply str
              (map (fn [entry]
                     (let [data (:data entry)
                           ...
                           text (cond
                                  (= :message (:kind entry)) (value/text-content (:message/content data))
...
(defn- exported [rt params]
  (let [packet (runtime/export! rt (sid params))
        ...
        text (case format
               ...
               (value/fail! :invalid-params "Export format must be jsonl, edn, or html" {:format format}))]
  ...
  (let [html (:content (exported rt (assoc params :format :html)))
        process (try
                  ...
                  (catch java.io.IOException error
                    (value/fail! :share-unavailable "GitHub CLI is required for explicit session sharing" {:cause (ex-message error)})))
    ...
    (try
      ...
      (when-not (.waitFor process 60 TimeUnit/SECONDS)
        ...
        (value/fail! :timeout "Session sharing timed out" {}))
      (let [text (str/trim @output)]
        (value/check! (zero? (.exitValue process)) :share-failed "GitHub rejected session sharing" {:detail text})
        (value/check! (re-find #"https://gist\.github\.com/\S+" text) :share-failed "GitHub did not return a gist URL" {:detail text})
        {:url text})
      (finally
...
(defn dispatch!
  ...
  [rt method params]
  (value/check! (map? params) :invalid-params "Command parameters must be an object" {})
  (case method
    ...
    "session.queue.update" {:item (runtime/update-queue! rt (sid params) (qid params)
                                                         (queue-content params))}
    "session.queue.drop" {:removed (runtime/drop-queue! rt (sid params) (qid params))}
    ...
    "session.import" (do
                       (value/check! (not (and (:path params) (:content params))) :invalid-params "Supply either path or content" {})
    ...
    (value/fail! :method-not-found "Unknown command" {:method method :methods methods})))
